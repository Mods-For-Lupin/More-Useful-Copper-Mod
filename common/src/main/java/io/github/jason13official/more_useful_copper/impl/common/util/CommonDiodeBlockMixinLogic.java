...
public class CommonDiodeBlockMixinLogic {
  ...
  /// Mixin injection: lets repeaters and comparators read copper wire power on their front input. Only overrides the return value when the existing result is less than 15 and the directly-faced
  /// neighbor is a copper wire.
  ///
  ...
  /// Mixin injection: lets comparators read copper wire power from their side inputs. Skipped when `sideInputDiodesOnly` is `true` (repeater mode) to prevent copper wire from incorrectly locking a
  /// repeater — only diode side-inputs should do that.
  ///
